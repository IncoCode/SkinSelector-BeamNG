...
function changeMatFile(vehicleDir, skinName) {
	//var code = 'function getSkinsFiles(%dir) {%i = 0; for( %file = findFirstFile( %dir @ "/*.dds" ); %file !$= ""; %file = findNextFile() ) { %files[%i] = %file; %i++; } return %files[0]; }';
	//var code = 'function getSkinsFiles(%dir) { %file = findFirstFile( %dir @ "/*.dds" ); %files[0] = "ss"; return %files[0]; }';
	var code = 'function getSkinsFiles(%vehicleDir, %skinDir) { %i = 0; for( %file = findFirstFile( %skinDir @ "/*.dds" ); %file !$= ""; %file = findNextFile() ) { %i++; %str = ""; %filename = strreplace( %file, %skinDir @ "/", "" ); %fileReader = new FileObject(); %fileReader.openForRead(%vehicleDir @ "materials.cs"); while ( !%fileReader.isEOF() ) { %tmp = %str; %line = %fileReader.readLine(); %str = %tmp NL %line; %text = strreplace( %str, %vehicleDir @ %filename, %file ); /*return %vehicleDir @ %filename @ " = " @ %file;*/ %str = %text; } %fileReader.close(); %fileReader = new FileObject(); %fileReader.openForWrite(%vehicleDir @ "materials.cs"); %fileReader.writeLine(%text); %fileReader.close(); return "yep"; } return "nope"; }';
